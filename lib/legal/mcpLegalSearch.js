import { getGoogleGenAIClient, isGoogleGenAiConfigured } from '../google/googleGenAiClient.js';
...
const AI_DYNAMIC_CANDIDATE_QUERY_LIMIT = Math.max(
    3,
    Math.min(10, Number(process.env.LEGAL_AI_DYNAMIC_CANDIDATE_QUERY_LIMIT || 6))
);
const AI_POOL_EXPANSION_QUERY_LIMIT = Math.max(
    2,
    Math.min(8, Number(process.env.LEGAL_AI_POOL_EXPANSION_QUERY_LIMIT || 5))
);
const AI_POOL_EXPANSION_TRIGGER_SCORE = Math.max(
    18,
    Math.min(60, Number(process.env.LEGAL_AI_POOL_EXPANSION_TRIGGER_SCORE || 30))
);
const AI_POOL_EXPANSION_TRIGGER_RESULT_COUNT = Math.max(
    4,
    Math.min(20, Number(process.env.LEGAL_AI_POOL_EXPANSION_TRIGGER_RESULT_COUNT || 8))
);
...
const buildSearchVariants = (signals = {}, resolvedSource = 'all') => {
    ...
    const phraseSeeds = [
        ...(signals.aiCandidateQueries || []),
        ...(signals.aiTopicHeadings || []),
        ...(signals.aiPhrases || []),
...
const normalizeAiSearchList = (values = [], {
    maxItems = 6,
    minLength = 4,
    maxLength = 96,
    requireSpace = false,
    ...
    for (const value of Array.isArray(values) ? values : []) {
        const compact = String(value || '').replace(/\s+/g, ' ').trim();
        const normalized = normalizeText(compact);
        if (!compact || compact.length < minLength || normalized.length < minLength) continue;
        if (requireSpace && !compact.includes(' ')) continue;
        if (seen.has(normalized)) continue;
        seen.add(normalized);
        ordered.push(compact.length > maxLength ? compact.slice(0, maxLength).trim() : compact);
...
const buildAiCandidatePoolQueries = ({
    aiPlan = null,
    query = '',
    aiExtractedPhrases = [],
} = {}) => {
    ...
    const pushQuery = (value = '') => {
        ...
        if (!compact || compact.length < 4 || normalized.length < 4 || seen.has(normalized)) return;
        seen.add(normalized);
        ordered.push(compact.length > 96 ? compact.slice(0, 96).trim() : compact);
    };

    for (const candidateQuery of normalizeAiSearchList(aiPlan?.candidateQueries, {
        maxItems: AI_DYNAMIC_CANDIDATE_QUERY_LIMIT,
        minLength: 6,
        maxLength: 96,
        requireSpace: true,
    })) {
        pushQuery(candidateQuery);
    }

    pushQuery(aiPlan?.shortQuery);
    pushQuery(aiPlan?.semanticQuery);

    for (const topicHeading of normalizeAiSearchList(aiPlan?.topicHeadings, {
        maxItems: 4,
        minLength: 4,
        maxLength: 56,
    })) {
        pushQuery(topicHeading);
    }

    const phraseSeeds = normalizeAiSearchList([
        ...(Array.isArray(aiPlan?.keywordPhrases) ? aiPlan.keywordPhrases : []),
        ...aiExtractedPhrases,
    ], {
        maxItems: 6,
        minLength: 4,
    ...
    if (phraseSeeds[0] && phraseSeeds[1]) {
        pushQuery(`${phraseSeeds[0]} ${phraseSeeds[1]}`);
    }
    if (phraseSeeds[0] && phraseSeeds[2]) {
        pushQuery(`${phraseSeeds[0]} ${phraseSeeds[2]}`);
    }

    return ordered.slice(0, AI_DYNAMIC_CANDIDATE_QUERY_LIMIT);
};

const buildAiSemanticSearchQuery = ({
    aiPlan = null,
    query = '',
    fullQuery = '',
    aiExtractedPhrases = [],
    ...
    const pushPart = (value = '') => {
        ...
        if (!compact || compact.length < 4 || normalized.length < 4 || seen.has(normalized)) return;
        seen.add(normalized);
        parts.push(compact.length > 140 ? compact.slice(0, 140).trim() : compact);
    };

    pushPart(aiPlan?.semanticQuery);
    pushPart(aiPlan?.shortQuery);
    ...
    for (const topicHeading of normalizeAiSearchList(aiPlan?.topicHeadings, {
        ...
        pushPart(topicHeading);
    }

    for (const phrase of normalizeAiSearchList([
        ...(Array.isArray(aiPlan?.keywordPhrases) ? aiPlan.keywordPhrases : []),
        ...aiExtractedPhrases,
    ], {
        maxItems: 4,
    ...
    pushPart(fullQuery || query);

    const combined = parts.join(' ; ').replace(/\s+/g, ' ').trim();
    return combined
        ? combined.slice(0, 240).trim()
        : String(fullQuery || query || '').replace(/\s+/g, ' ').trim();
};

const buildPoolExpansionQueries = ({
    query = '',
    aiCandidateQueries = [],
    variantQueries = [],
} = {}) => {
    ...
    for (const value of aiCandidateQueries) {
        pushQuery(value);
    }
    for (const value of variantQueries) {
        pushQuery(value);
    }

    return ordered.slice(0, AI_POOL_EXPANSION_QUERY_LIMIT);
};
...
export const extractSearchPhrasesWithAI = async (queryText) => {
    try {
        const ai = getLegalAiClient();
        if (!ai) {
            console.log('[LEGAL_SEARCH] Google GenAI is not configured, skipping AI phrase extraction.');
            return [];
        }
...
- semanticQuery en fazla 18 kelime olsun
...
- Kisi adi, sirket adi, adres, ada/parsel, tarih, belge no gibi detaylari shortQuery veya candidateQueries icine alma.
...
- shortQuery ve semanticQuery karar aramada gecmesi muhtemel resmi hukuki ifadelerden kurulsun.
...
JSON:
{
  ...
  "semanticQuery": "hukuki cekirdegi anlatan tek cumlelik sorgu",
...
"""${String(queryText || '').trim()}"""`;
        ...
        return {
            ...
            shortQuery: String(parsed.shortQuery || '').replace(/\s+/g, ' ').trim().slice(0, 120),
            semanticQuery: String(parsed.semanticQuery || '').replace(/\s+/g, ' ').trim().slice(0, 180),
            topicHeadings: normalizeAiSearchList(parsed.topicHeadings, {
                maxItems: 4,
                minLength: 4,
                maxLength: 56,
            }),
            candidateQueries: normalizeAiSearchList(parsed.candidateQueries, {
                maxItems: AI_DYNAMIC_CANDIDATE_QUERY_LIMIT,
                minLength: 6,
                maxLength: 96,
                requireSpace: true,
            }),
            keywordPhrases: normalizeAiSearchList(parsed.keywordPhrases, {
                maxItems: 10,
                minLength: 4,
                maxLength: 64,
                requireSpace: true,
...
export const searchLegalDecisionsViaMcp = async ({
    ...
    let aiCandidateQueries = [];
    let aiSemanticQuery = fullQuery;
    ...
    if (searchArea !== 'auto' && SEARCH_AREA_CONFIG[searchArea]) {
        ...
        aiCandidateQueries = buildAiCandidatePoolQueries({
            aiPlan,
            query,
            aiExtractedPhrases,
        });
        aiSemanticQuery = buildAiSemanticSearchQuery({
            aiPlan,
            query,
            fullQuery,
            aiExtractedPhrases,
    ...
    if (aiCandidateQueries.length > 0) {
        signals.aiCandidateQueries = aiCandidateQueries.slice(0, AI_DYNAMIC_CANDIDATE_QUERY_LIMIT);
    }
    if (Array.isArray(aiPlan?.topicHeadings) && aiPlan.topicHeadings.length > 0) {
        signals.aiTopicHeadings = aiPlan.topicHeadings.slice(0, 4);
    }
    ...
    const variantQueries = buildSearchVariants(signals, resolvedSource);
    const poolExpansionQueries = buildPoolExpansionQueries({
        query,
        aiCandidateQueries,
        variantQueries,
    });
    ...
    if (
        (topScore < AI_POOL_EXPANSION_TRIGGER_SCORE || rawResults.length < AI_POOL_EXPANSION_TRIGGER_RESULT_COUNT) &&
        poolExpansionQueries.length > 0
    ) {
        const expansionGroups = await Promise.all(
            poolExpansionQueries.map((expansionQuery) =>
        ...
        rawResults = dedupeDecisions([...rawResults, ...expansionGroups.flat()]);
        rawResults = applySearchAreaFilters(rawResults, searchArea);
        scoredResults = scoreDecisionResults(rawResults, signals);
        warningParts.push(
            aiCandidateQueries.length > 0
                ? 'AI destekli alternatif sorgularla aday havuzu genisletildi.'
                : 'Anahtar ifade varyantlariyla ek MCP aramasi yapildi.'
        );
    }

    const postExpansionTopScore = Number(scoredResults[0]?.relevanceScore || 0);
    const semanticTriggerScore = getSemanticTriggerScore(signals, resolvedSource);
    if (
        USE_MCP_SEMANTIC_SEARCH &&
        resolvedSource !== 'anayasa' &&
        postExpansionTopScore < semanticTriggerScore
    ) {
        try {
            const semanticResults = await searchSemanticBedesten({
                initialKeyword: query,
                semanticQuery: aiSemanticQuery || fullQuery,
                source: resolvedSource,
                filters: effectiveFilters,
