...
const normalizeAnalyzerSearchText = (value = '') =>
    String(value || '').replace(/\s+/g, ' ').trim();

const ANALYZER_COMMAND_PATTERNS = [
...
const ANALYZER_DIRECTION_TOKENS = new Set([
...
const ANALYZER_META_PREFIXES = [
    /^analiz edilen metin(?: parcasi)?/i,
    /^sunulan metin(?: parcasi)?/i,
    /^bu metin/i,
    /^ilgili metin/i,
];

const ANALYZER_META_SIGNAL_REGEX = /\b(?:hukuku alaninda|hakliligini vurgulayan|guclu ve zayif yonleri|strateji gelistir|strateji oluştur|strateji olustur)\b/i;

const stripAnalyzerInstructionPhrases = (value = '') => {
    let text = normalizeAnalyzerSearchText(value);
    for (const pattern of ANALYZER_COMMAND_PATTERNS) {
        text = text.replace(pattern, ' ');
    }
    return text.replace(/\s+/g, ' ').trim();
};

const isDirectionOnlyAnalyzerText = (value = '') => {
    const tokens = normalizeAnalyzerSearchText(value)
    ...
    const hasDirectionToken = tokens.some((token) => ['lehine', 'aleyhine', 'yararina', 'zararina'].includes(token));
    if (!hasDirectionToken) return false;
    return tokens.every((token) => ANALYZER_DIRECTION_TOKENS.has(token));
};

export const sanitizeAnalyzerSearchText = (value = '') => {
    const normalized = normalizeAnalyzerSearchText(value);
    if (!normalized) return '';
    if (ANALYZER_META_PREFIXES.some((pattern) => pattern.test(normalized)) && ANALYZER_META_SIGNAL_REGEX.test(normalized)) {
        return '';
    }
    const stripped = stripAnalyzerInstructionPhrases(normalized);
    if (!stripped) return '';
    if (isDirectionOnlyAnalyzerText(stripped)) return '';
    return stripped;
};
...
export const buildDocumentAnalyzerResult = (analysisData, fallbackText = '') => {
    ...
    const sanitizedFallbackText = sanitizeAnalyzerSearchText(fallbackText);
    const sanitizedSummary = sanitizeAnalyzerSearchText(analysisData.summary || '');
    const fallbackQuery = sanitizedFallbackText || sanitizedSummary || '';
