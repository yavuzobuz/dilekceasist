...
const DATASET_PATH = path.resolve(__dirname, 'yargi-cli-longform-25-extra.json');
const OUTPUT_JSON = path.resolve('output/yargi-cli-longform-25-extra-results.json');
const OUTPUT_MD = path.resolve('output/yargi-cli-longform-25-extra-report.md');
...
const toMarkdown = ({ summary, records }) => {
  const lines = [
    '# Yargi CLI Longform 25 Extra Eval',
    '',
    `- Total: ${summary.total}`,
...
const main = async () => {
  ...
  for (const scenario of dataset) {
    ...
    const record = {
      ...
      error,
      topResults: results.slice(0, 10).map((item) => topResultShape(item)),
    };
