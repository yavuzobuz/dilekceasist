...
export const searchLegalDecisions = async ({
    ...
    // rawQuery: kullanıcının tam orijinal sorgusu — rerank ve routing için korunur
    ...
    const retries = [
        endpoint,
        `${endpoint}?retry=1`,
        `${endpoint}?retry=2` /* Removed the action fallback as it's not handled by the new server path */,
    ];
